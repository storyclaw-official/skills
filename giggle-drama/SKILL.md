...
**重试场景（用户说"重试"/"再试一次"，或子步骤失败后用户确认重新生成）：跳过故事/时长/风格确认（复用原参数），但必须提醒用户"您可以去网页端专业模式继续完成视频制作；如果选择重试，将创建新任务并再次消耗积分，之前的费用不退。是否重试？"，用户确认后执行 start。**
...
| 子步骤失败 | 告知步骤名和错误信息，提醒"您可以去网页端专业模式继续完成视频制作；如果选择重试，将创建新任务并再次消耗积分，之前的费用不退"，询问用户是否重新生成。确认后执行 `start`（唯一重试方式，API 不支持断点续传）：`python3 scripts/trustee_api.py start --story "..." --aspect "..." --project-name "..."` |
