...
description: 当用户想通过 StoryClaw 在社交媒体上发帖、定时发布、查询、管理或分析帖子时使用本技能。支持 TikTok、Instagram、Facebook、X/Twitter、YouTube、LinkedIn、Threads、Pinterest、Reddit、Bluesky、Telegram、Snapchat、Google Business Profile 共 13 个平台。
---
...
首先询问用户：
> "在开始之前，请确认你是否已经在 StoryClaw.com 的个人中心绑定了你的社交账号？"

- 已绑定 → 进入阶段二
- 未绑定 → 引导前往 StoryClaw.com → 登录 → 个人中心完成绑定

---

## 阶段二：获取并验证 API Key

> "请前往 StoryClaw.com → 个人中心 → StoryClaw API Key，复制你的 Key 粘贴给我（格式：`sk_xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx`）。"

调用 `get_social_status` 验证并获取已绑定平台列表：
